// waiter.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
#include <iostream>
#include <mutex>
#include <random>
...
public:
    ...
    void PickFork(unsigned int id) {
        bool x = forks_[id].try_lock();
        bool y = forks_[(id + 1) % kNumberPhilosophers].try_lock();
        while (!(x && y)) {
            if (x)
                forks_[id].unlock();
            if (y)
                forks_[(id + 1) % kNumberPhilosophers].unlock();
            x = forks_[id].try_lock();
            y = forks_[(id + 1) % kNumberPhilosophers].try_lock();
        }
    }
